...
const app = Vue.createApp({
    ...
    computed: {
        baseRows() {
            ...
            return this.rows.filter(i => (i['提案人']||'').includes(this.personFilter))
        },
        hasAnyFilter() {
        ...
        filteredData() {
            return this.baseRows.filter(i => {
                ...
                if (kw) {
                    const fields = ['日期','棟別','站點','組織類別','案件分類','項目描述','項目Due Date','距今','當前最新進度']
                ...
                return mDate&&mBldg&&mSite&&mOrg&&mCase&&mDesc&&mDue&&mAgo
            })
        },
        ...
        currentChecked() {
            const m = {'日期':this.checkedDates,'棟別':this.checkedBuildings,'站點':this.checkedSites,
                       ...
                       '項目Due Date':this.checkedDues,'距今':this.checkedAgos}
        ...
        currentUniqueAll() {
            const m = {'日期':this.uniqueDates,'棟別':this.uniqueBuildings,'站點':this.uniqueSites,
                       ...
                       '項目Due Date':this.uniqueDues,'距今':this.uniqueAgos}
            return m[this.activeFilter] || []
        },
        currentOptions() {
    ...
    methods: {
        ...
        clearCurrentFilter() {
            const m={'日期':'checkedDates','棟別':'checkedBuildings','站點':'checkedSites',
                     ...
                     '項目Due Date':'checkedDues','距今':'checkedAgos'}
        ...
        resetAllFilters() {
            ...
            this.checkedDues=[]; this.checkedAgos=[]; this.keyword=''
            this.closeFilter()
        },
        ...
        async submitAdd() {
            ...
            try {
                await axios.post(API+'/api/add', {...this.addForm, '提案人': this.account})
